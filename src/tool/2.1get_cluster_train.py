...
# 读入数据集
# dataset = 'ml-1m'
dataset = 'Grocery_and_Gourmet_Food'  # 'Digital_Music' 'Office_Products' 'Clothing_Shoes_and_Jewelry' 'Kindle_Store' 'Pet_Supplies' 'Beauty' 'Grocery_and_Gourmet_Food'
path = r'../../data/' + dataset

if dataset == 'RecSys':
    item_meta_df = pd.read_csv(path + r'/item_meta_2.csv', sep='\t')
    # 转为one-hot
    X_to_code = item_meta_df.iloc[:, 1:]
    onehotencoder = OneHotEncoder()
    X_one = onehotencoder.fit_transform(X_to_code).toarray()
    # 把转换后的数据加上原来的第一列，并转回DataFrame
    column_names = onehotencoder.get_feature_names_out(input_features=item_meta_df.columns[1:])
    df_encoded = pd.DataFrame(X_one, columns=column_names)
    df_encoded = pd.concat([item_meta_df.iloc[:, 0].reset_index(drop=True), df_encoded], axis=1)
    item_meta_df = df_encoded.copy()
    item_meta_df.to_csv(path + r'/item_meta.csv', sep='\t')
else:
