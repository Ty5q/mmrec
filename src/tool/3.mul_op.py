...
if __name__ == '__main__':
    ...
    method = 'SASRec'  # 'GRU4Rec' 'ComiRec' 'SLRCPlus' 'SASRec'
    # 读入数据集
    path = r'../../data/' + dataset
    # path = r'../../data/RecSys'
    ...
    # 读入聚类中心数据
    res_dict = {}
    cluster_center_df = pd.read_csv(path + "/cluster_center_info_vec_6.csv", sep='\t')
    print("聚类中心数据已读取")
    center_user_list = cluster_center_df['user_id'].values
    #
    for k in n_var:
        filename = f'../res_{dataset}/{dataset}_{method}_moea_{k}_center_vec_6.csv'
